...
def parseCommandLineArguments():
    ...
    ap.add_argument("-f", "--config", required=True,
                    help="path to config for input video")
    ...
    inputVideoConfigPath = args["config"]
    outputVideoPath = args["output"]
    confidence = args["confidence"]
    threshold = args["threshold"]
    USE_GPU = args["use_gpu"]

    f = open(inputVideoConfigPath)
    trackingLines = json.load(f)

    return LABELS, weightsPath, configPath, inputVideoPath, trackingLines, outputVideoPath, confidence, threshold, USE_GPU
